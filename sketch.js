...
if(gameState===PLAY){
   ...
   //fruitGroup.setVelocityYEach(-10)
   for (var i = 0;i < fruitGroup.length;i++){
     if(fruitGroup[i].y < 300){
       console.log(fruitGroup[i].y)
       fruitGroup[i].velocityY += 2
     }
   }
   for (var i = 0;i < monsterGroup.length;i++){
    if(monsterGroup[i].y < 300){
      console.log(monsterGroup[i].y)
      monsterGroup[i].velocityY += 2
    }
  }
...
function Monster(){
  if(frameCount%200===0){
    monster=createSprite(400,600,20,20);
